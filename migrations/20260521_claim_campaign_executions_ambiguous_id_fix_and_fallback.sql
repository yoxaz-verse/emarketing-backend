-- Fix ambiguous id references in claim RPC and add resilient fallback claim RPC.
...
drop function if exists public.claim_campaign_executions_fallback(text, integer);
drop function if exists public.claim_campaign_executions_fallback(uuid, integer);
...
begin
  ...
  with locked as (
    ...
    where cl.id in (select locked.id from locked)
...
-- Fallback claim path used by backend runtime when primary claim RPC drifts/errors.
create or replace function public.claim_campaign_executions_fallback(
  p_campaign_id uuid,
  p_limit integer
)
returns table (
